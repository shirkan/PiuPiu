...
bool js_pluginx_protocols_ProtocolAds_preLoadInterstitial(JSContext *cx, uint32_t argc, jsval *vp)
{
    JSObject *obj = JS_THIS_OBJECT(cx, vp);
    js_proxy_t *proxy = jsb_get_js_proxy(obj);
    cocos2d::plugin::ProtocolAds* cobj = (cocos2d::plugin::ProtocolAds *)(proxy ? proxy->ptr : NULL);
    JSB_PRECONDITION2( cobj, cx, false, "js_pluginx_protocols_ProtocolAds_preLoadInterstitial : Invalid Native Object");
    if (argc == 0) {
        cobj->preLoadInterstitial();
        JS_SET_RVAL(cx, vp, JSVAL_VOID);
        return true;
    }
    
    JS_ReportError(cx, "js_pluginx_protocols_ProtocolAds_preLoadInterstitial : wrong number of arguments: %d, was expecting %d", argc, 0);
    return false;
}
...
void js_register_pluginx_protocols_ProtocolAds(JSContext *cx, JSObject *global) {
    ...
    static JSFunctionSpec funcs[] = {
        ...
        JS_FN("preLoadInterstitial", js_pluginx_protocols_ProtocolAds_preLoadInterstitial, 0, JSPROP_PERMANENT | JSPROP_ENUMERATE),
